...
class Commands {
  ...
  docslink(message, channelID, url) {
    ...
    console.log(`Linking docs for ${repo} to ${url}`);

    if (!url || !url.length) {
      delete repo.docsURL;
      message.channel.sendMessage(`${config.emojis.success} Successfully **unlinked** docs website.`);
    }
    else {
      repo.docsURL = url;
      message.channel.sendMessage(`${config.emojis.success} Successfully **linked** docs website.`);
    }
  }
