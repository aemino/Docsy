...
class Commands {
  ...
  init(message, owner, repo, branch, path) {
    ...
    request.post(`https://api.github.com/repos/${owner}/${repo}/hooks`, {headers: config.github.headers, json: webhookData}, (err, response, body) => {
      if (err) {
  ...
  setActive(active, message, channelID) {
    ...
    request.patch(`https://api.github.com/repos/${repo.owner}/${repo.repo}/hooks/${hookID}`, {headers: config.github.headers, json: webhookData}, (err) => {
      if (err) {
        console.log(err);
  ...
  remove(message, channelID) {
    ...
    request.delete(`https://api.github.com/repos/${repo.owner}/${repo.repo}/hooks/${hookID}`, {headers: config.github.headers}, (err) => {
      if (err) {
        console.log(err);
