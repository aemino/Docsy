...
class Lookup {
  ...
  respond(message, params) {
    ...
    lookupClass = classes.get(lookupClass) || interfaces.get(lookupClass) || typedefs.get(lookupClass);
  ...
  generateClass(docsClass) {
    ...
    const construct = docsClass.classConstructor.directData.params.map(param => this.generateParam(param)).join(', ');
  ...
  generateTypeDef(docsTypeDef) {
    ...
    const type = this.generateVarType(docsTypeDef.type);
