...
class Lookup {
  ...
  respond(message, params) {
    ...
    lookupClass = classes.get(lookupClass) || interfaces.get(lookupClass);
    ...
    if (lookupClass && lookupSub) {
      ...
      lookupSub = props.get(lookupSub) || methods.get(lookupSub) || events.get(lookupSub);
    }
  ...
  generateMember(docsMember) {
    ...
    if (docsMember.directData.type.directData) {
      typeName = "type";
      type = this.generateVarType(docsMember.directData.type);
    }
    else if (docsMember.directData.returns) {
      typeName = "returns";
      type = docsMember.directData.returns.map(returns => this.generateVarType(returns.type)).join(', ');
    }
