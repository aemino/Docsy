...
class Docs {
  ...
  init(repo) {
    ...
    return new Promise((resolve, reject) => {
      if (fs.existsSync(`./repos/${repoData.repo}`)) return resolve();

      console.log(`Cloning repo ${repo}...`);

      try {
        child_process.spawnSync(`git clone -b ${repoData.branch} --single-branch https://github.com/${repoData.owner}/${repoData.repo}.git`, {
